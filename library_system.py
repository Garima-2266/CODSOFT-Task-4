...
def issuer_card():
    ...
    if not Cid:
        mb.showerror('Issuer ID cannot be zero!','Can\'t keep Issuer ID empty, it should have a value')
    else:
        return Cid
...
def add_record():
    ...
    surety = mb.askyesno('Are you sure?','Are you sure this is the data you want to enter?'
             '\nPease note that Book ID cannot be changed in the future')
    ...
    if surety:
        try:
            ...
            mb.showerror('Book ID already in use!',
                         ...
                         'please alter that book\'s record')
...
root = Tk()
root.title('CODSOFT Library Management System')
root.geometry('1010x700')
root.resizable(0, 0)
root.config(bg=bg_color)


Label(root, text='LIBRARY MANAGEMENT SYSTEM', font=("Arial", 15, 'bold'), bg=btn_hlb_bg, fg='White').pack(side=TOP, fill=X)
...
left_frame = Frame(root, bg=frame_bg_color)
left_frame.place(x=0, y=30, relwidth=0.3, relheight=0.96)


RT_frame = Frame(root, bg=frame_bg_color)
RT_frame.place(relx=0.3, y=30, relheight=0.2, relwidth=0.7)


RB_frame = Frame(root, bg=frame_bg_color)
RB_frame.place(relx=0.3, rely=0.24, relheight=0.785, relwidth=0.7)


Label(left_frame, text='Book Name', bg=frame_bg_color, font=label_font).place(x=98, y=25)
Entry(left_frame, width=25, font=entry_font, text=bk_name).place(x=45, y=55)

Label(left_frame, text='Book ID', bg=frame_bg_color, font=label_font).place(x=110, y=105)
...
Button(RT_frame, text='Delete book record', font=button_font, bg=button_bg_color, width=17, command=remove_record).place(x=8, y=30)
Button(RT_frame, text='Delete full inventory', font=button_font, bg=button_bg_color, width=17, command=delete_inventory).place(x=178, y=30)
Button(RT_frame, text='Update book details', font=button_font, bg=button_bg_color, width=17, command=update_record).place(x=348, y=30)
Button(RT_frame, text='Change Book Availability', font=button_font, bg=button_bg_color, width=19, command=change_availability).place(x=518, y=30)

Label(RB_frame, text='BOOK INVENTORY', bg=rbf_bg, font=("Arial", 15, 'bold')).pack(side=TOP, fill=X)
